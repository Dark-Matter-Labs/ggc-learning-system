...
interface SimpleReviewClientProps {
  ...
  readonly onNodeUpdate?: (updates: { title: string; description: string }) => void;
}

export function SimpleReviewClient({ node, onPromote, onArchive, isSubmitting, onNodeUpdate }: SimpleReviewClientProps) {
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      onNodeUpdate?.({ title, description: editDescription.trim() });
      setIsEditing(false);
    } catch {
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {!isEditing && (
          <button
            ...
            className="text-xs text-gray-400 hover:text-gray-200 border border-gray-700 hover:border-gray-500 px-2 py-1 rounded transition-colors"
          >
            Edit
          </button>
        )}
        ...
        <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-[#185FA5]"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-3 py-1.5 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-[#185FA5] resize-none"
            />
          </div>
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-[#185FA5] text-white rounded disabled:opacity-50"
            >
              {isSavingEdit && <Spinner size="sm" />}
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs text-gray-500 hover:text-gray-300 border border-gray-700 rounded disabled:opacity-50"
            >
              Cancel
            </button>
          </div>
